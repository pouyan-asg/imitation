...
class BaseImitationAlgorithm(abc.ABC):
    """Base class (abstract) for all imitation learning algorithms."""
        ...
        *,
        ...
        Args:
            ...
            allow_variable_horizon: If False (default), algorithm will raise an
                ...
                WARNING: variable horizon episodes leak information about the reward via termination
        ...
        self._horizon = None  # Start with no known trajectory horizon.

    # Getter for the logger — allows code to use self.logger.
    @property
    def logger(self) -> imit_logger.HierarchicalLogger:
        return self._logger

    # Setter for replacing the logger cleanly.
    @logger.setter
    ...
    def _check_fixed_horizon(self, horizons: Iterable[int]) -> None:
        ...
        (i.e. all trajectories have the same length)
        ...
        # 'set' in Python is an unordered collection of unique elements and it removes duplicates.
        # The .add() method adds a new element to the set — if it’s not already there.
        horizons = set(horizons)
        if self._horizon is not None:
            horizons.add(self._horizon)

        # If there’s more than one unique length, error.
        if len(horizons) > 1:
            raise ValueError(
    ...
    """
    Custom save/load behavior to avoid pickle issues
    
    __dict__: a dictionary that stores all the instance's attributes.
            Remove the _logger from the state because loggers usually 
            cannot be saved (e.g. they hold open file handles).
                _logger is already defined in __init__
    """
