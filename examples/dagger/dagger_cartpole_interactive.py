...
"""
max_episode_steps: total number of steps in one episode which means total number that expert gives an action.
rollout_round_min_episodes: This is part of the DAgger training loop. In each round, DAgger will collect at least 'rollout_round_min_episodes' full episode from the expert (with some agent interaction too).
rollout_round_min_timesteps: Each DAgger round must collect at least 'rollout_round_min_timesteps' steps total, no matter how many episodes it takes.
total_timesteps: total number of all episodes that the agent will train. Training will loop until it has seen at least 'total_timesteps total environment steps (not policy steps, env steps).
...
n_eval_episodes: This controls how many test episodes are run after training is done. Each episode normally contains 'max_episode_steps' steps.
"""
...
# save_stable_model(Path("my_saved_policy"), dagger_trainer.policy)
from imitation.util import util

policy_path = "policy-latest.pt"
util.save_policy(dagger_trainer.policy, policy_path)
